...
def main(args: argparse.Namespace) -> int:
    if args.check:
        if not OUTPUT.exists() or OUTPUT.read_text(encoding="utf-8") != rendered_manifest:
            print(f"stale manifest: regenerate {OUTPUT} with {Path(__file__).name}")
            return 1
        print(f"verified {len(files)} mappings in {OUTPUT}")
        return 0

    OUTPUT.write_text(rendered_manifest, encoding="utf-8")
    print(f"wrote {len(files)} mappings to {OUTPUT}")
    return 0
