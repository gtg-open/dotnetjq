...
def implementation_names(metadata: Mapping[str, object]) -> tuple[str, ...]:
    baseline = metadata.get("paired_baseline")
    if baseline is None:
        return IMPLEMENTATION_NAMES
    if not isinstance(baseline, dict) or baseline.get("mode") != "same-process-interleaved":
        raise ReportOutputError("invalid paired baseline mode")
    source = baseline.get("source")
    commit = baseline.get("commit")
    if (not isinstance(commit, str) or len(commit) != 40
            or any(c not in "0123456789abcdef" for c in commit)
            or not isinstance(source, dict) or source.get("git_head") != commit
            or source.get("git_dirty") is not False):
        raise ReportOutputError("invalid paired baseline source identity")
    attestations = baseline.get("attestations")
    if (not isinstance(attestations, list) or len(attestations) != 2
            or not all(isinstance(item, dict) for item in attestations)
            or [item.get("deployment") for item in attestations]
            != ["framework-dotnetjq", "aot-dotnetjq"]):
        raise ReportOutputError("invalid paired baseline attestation inventory")
    return (*IMPLEMENTATION_NAMES, "baseline-framework-dotnetjq", "baseline-aot-dotnetjq")
...
def _validate_implementation_metadata(metadata: Mapping[str, object], kind: str) -> None:
    ...
    if (
        ...
        != list(implementation_names(metadata))
    ):
    ...
    if len(set(hashes[:3])) != 3 or (len(hashes) == 5 and len(set([*hashes[3:], hashes[2]])) != 3):
        raise ReportOutputError(f"{kind} result executable identities are not distinct")
    if len(hashes) == 5:
        paths = [item.get("executable_path") for item in implementations]
        if not all(isinstance(path, str) and path for path in paths) or len(set(paths)) != 5:
            raise ReportOutputError("paired baseline executable paths are not distinct")
...
def _validate_fixture_report(directory: pathlib.Path) -> dict[str, object]:
    ...
    names = implementation_names(metadata)
    ...
    expected_summaries = {
        ...
        ("startup:null", name): startup_repetitions for name in names
    }
    ...
    if aggregate_keys != [
        (group, name) for group in expected_groups for name in names
    ]:
    ...
    timed_measurements = scenario_count * len(names) * repetitions + (
        len(names) * startup_repetitions
    )
    return {
        "schema_version": 1,
        "kind": "fixture",
...
def _validate_macro_report(directory: pathlib.Path) -> dict[str, object]:
    ...
    if aggregate_keys != [
        (group, group, name) for group in expected_groups for name in names
    ]:
