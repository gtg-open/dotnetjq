...
GitHub-hosted jobs may add `--github-hosted` when inspecting generated outputs.
The option is rejected unless `GITHUB_ACTIONS=true` and
`RUNNER_ENVIRONMENT=github-hosted`, as defined in GitHub's
[variables reference](https://docs.github.com/en/actions/reference/workflows-and-actions/variables).
It recognizes only the standard Linux and macOS homes for the `runner` account
and the Windows C-drive home for `runneradmin`, documented in GitHub's
[PowerShell module locations](https://docs.github.com/en/actions/tutorials/build-and-test-code/powershell#powershell-module-locations).
These are public hosted infrastructure paths. They can legitimately appear in
build provenance; recognizing them avoids changing signed or hashed evidence.
An image change to another home prefix requires a reviewed policy change.

This option affects only the home-path rule for explicitly selected outputs.
Repository/index/history inspection remains strict. It does not permit other
usernames, similarly prefixed accounts, parent traversal out of those homes,
self-hosted runner paths, secrets, private endpoints, or private denylist values.
The default remains strict, including for local runs. The environment values
are an operational check, not cryptographic host attestation; maintainers must
keep the workflow on the configured GitHub-hosted runners and must never set
those variables to disguise a local or self-hosted execution.
