...
MAX_DEPTH = 4
# Public hosted image locations documented by GitHub, reviewed 2026-09-08:
# https://docs.github.com/en/actions/tutorials/build-and-test-code/powershell#powershell-module-locations
# Split literals keep the strict source scan independent of output exemptions.
PUBLIC_HOSTED_HOMES = ("/" + "home/runner", "/" + "Users/runner", "C:/" + "Users/runneradmin")
...
class Scanner:
    def __init__(self, denied: tuple[str, ...] = (), *, github_hosted: bool = False):
        if github_hosted and not hosted_environment():
            raise ScanError("github-hosted-environment-required")
        self.github_hosted = github_hosted
        self.denied = denied
    ...
    def matches(self, text: str) -> set[str]:
        ...
        for name, pattern in RULES:
            if name == "workstation-home-path" and self.github_hosted:
                if any(not self.public_hosted_home(match, text) for match in pattern.finditer(text)):
                    found.add(name)
    ...
    @staticmethod
    def public_hosted_home(match: re.Match[str], text: str) -> bool:
        value = re.sub(r"[\\/]+", "/", match.group())
        if value not in PUBLIC_HOSTED_HOMES[:2] and value.casefold() != PUBLIC_HOSTED_HOMES[2].casefold():
            return False
        # Never accept a parent traversal that can leave the public home root.
        tail = re.match(r"[^\s\"'<>;,:]*", text[match.end():]).group()
        return ".." not in re.split(r"[\\/]+", tail)
            ...
            if history:
                for record in git(root, "ls-tree", "-r", "-z", revision.decode()).split(b"\0"):
                    ...
                    if object_type == b"blob" and (oid, name) not in inspected_blobs:
                        ...
                        # Retain repository-relative filenames for useful,
                        # consistently redacted diagnostics across history.
...
def main() -> int:
    ...
    parser.add_argument("--github-hosted", action="store_true", help="recognize fixed public hosted home roots in outputs only; requires a GitHub-hosted Actions environment")
    args = parser.parse_args()
    try:
        if args.github_hosted and not hosted_environment():
            raise ScanError("github-hosted-environment-required")
        ...
        # Repository contents and history are always subject to the strict rule.
        # This option recognizes infrastructure paths in generated provenance;
        # it never edits the bytes being inspected or their certificates.
        scanner.github_hosted = args.github_hosted
