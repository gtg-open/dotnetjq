...
from check import Scanner, ScanError
...
class PrivacyTests(unittest.TestCase):
    ...
    def test_hosted_option_requires_both_environment_values(self):
        for actions, environment in [("", ""), ("true", ""), ("", "github-hosted"),
                                     ("true", "self-hosted"), ("false", "github-hosted")]:
            with self.subTest(actions=actions, environment=environment):
                self.env.update(GITHUB_ACTIONS=actions, RUNNER_ENVIRONMENT=environment)
                self.assertEqual(self.scan("--github-hosted").returncode, 2)
                with patch.dict(os.environ, {"GITHUB_ACTIONS": actions, "RUNNER_ENVIRONMENT": environment}):
                    with self.assertRaises(ScanError):
                        Scanner(github_hosted=True)

    def test_hosted_option_only_permits_exact_public_homes(self):
        homes = ["/" + "home/runner/", "/" + "Users/runner/", "C:/" + "Users/runneradmin/",
        ...
        with patch.dict(os.environ, {"GITHUB_ACTIONS": "true", "RUNNER_ENVIRONMENT": "github-hosted"}):
            for home in homes:
                with self.subTest(home=home):
                    self.assertIn("workstation-home-path", Scanner().matches(home + "build/output"))
                    self.assertFalse(Scanner(github_hosted=True).matches(home + "build/output"))
                    self.assertIn("workstation-home-path", Scanner(github_hosted=True).matches(home + "build/output\n" + SYNTHETIC_HOME))
                    self.assertIn("workstation-home-path", Scanner(github_hosted=True).matches(home + "../another-account"))
            for home in ["/" + "home/runner-personal/x", "/" + "Users/runneradmin/x",
                         "C:/" + "Users/runneradmin-personal/x", "D:/" + "Users/runneradmin/x",
                         "/" + "home/Runner/x", "/" + "Users/Runner/x"]:
                self.assertIn("workstation-home-path", Scanner(github_hosted=True).matches(home))

    def test_hosted_paths_do_not_disable_any_other_rule_or_private_denylist(self):
        home = "/" + "home/runner/build/"
        with patch.dict(os.environ, {"GITHUB_ACTIONS": "true", "RUNNER_ENVIRONMENT": "github-hosted"}):
            self.assertIn("github-token", Scanner(github_hosted=True).matches(home + SYNTHETIC_TOKEN))
            self.assertIn("private-network-address", Scanner(github_hosted=True).matches(home + "10." + "23.45.67"))
            self.assertIn("private-denylist-match", Scanner((home,), github_hosted=True).matches(home + "output.json"))

    def test_hosted_cli_does_not_exempt_repository_content(self):
        home = "/" + "home/runner/build/output"
        self.env.update(GITHUB_ACTIONS="true", RUNNER_ENVIRONMENT="github-hosted")
        self.write("generated.json", home)
        self.assertEqual(self.scan("--github-hosted", "--output", str(self.root / "generated.json")).returncode, 0)
        self.git("add", "generated.json")
        self.assertEqual(self.scan("--github-hosted", "--output", str(self.root / "generated.json")).returncode, 1)
        # The option can also accompany an empty optional-output inventory.
        self.git("rm", "--cached", "generated.json")
        self.assertEqual(self.scan("--github-hosted").returncode, 0)
