...
for command in awk python3 tar; do
...
package_sha256=$(release_sha256 "$artifacts/$name")
payload_sha256=$(python3 "$release_script_dir/verify-nuget-package-payload.py" \
  --package "$artifacts/$name" --package-id "$package_id" \
  --version "$version" --signature-policy absent)
printf '%02d\tPOINTER_LAST\t%s\t%s\t%s\t%s\t%s\n' \
  "$order" "$package_id" "$version" "$name" "$package_sha256" \
  "$payload_sha256" >>"$expected_plan"
cmp -s -- "$expected_plan" "$publication_plan" ||
  release_die 'NuGet publication plan does not exactly match the ten verified identities, packages, and hashes'
