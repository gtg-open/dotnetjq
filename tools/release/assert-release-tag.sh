...
tag_type=$(git -C "$release_repository_root" cat-file -t "$tag_ref" 2>/dev/null) ||
  release_die "local release tag does not exist: $tag"
[ "$tag_type" = tag ] ||
  release_die "release tag must be an annotated tag object, not a lightweight tag: $tag"
tag_target_type=$(git -C "$release_repository_root" cat-file -p "$tag_ref" |
  sed -n 's/^type //p')
[ "$tag_target_type" = commit ] ||
  release_die "annotated release tag must point directly to a commit: $tag"
local_tag_object=$(git -C "$release_repository_root" rev-parse "$tag_ref") ||
  release_die "could not resolve local annotated tag object: $tag"
git -C "$release_repository_root" fetch --force --no-tags "$remote" \
  "refs/heads/$main_branch:refs/remotes/$remote/$main_branch" >/dev/null
local_commit=$(git -C "$release_repository_root" rev-parse "$tag_ref^{commit}") ||
...
remote_commit=$(printf '%s\n' "$remote_refs" |
  ...
  release_die "remote release tag is not an annotated tag: $tag"
[ "$remote_tag_object" = "$local_tag_object" ] ||
  release_die "remote annotated tag object $tag changed from $local_tag_object to $remote_tag_object"
