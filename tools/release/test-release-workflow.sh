...
job_contains "$core_publish_job" 'scopes != {"public_repo"}' ||
  release_die 'the WinGet token preflight must reject every scope set except public_repo'
...
job_contains_in_order "$core_publish_job" 'Require GitHub release immutability' 'id: homebrew-preflight-token' ||
