...
publisher_yaml="'$(printf '%s' "$publisher" | sed "s/'/''/g")'"
...
release_render_template "$release_repository_root/packaging/winget/version.yaml.template" "$work_dir/$package_id.yaml" \
  __PACKAGE_ID__ "$package_id" __VERSION__ "$version"
release_render_template "$release_repository_root/packaging/winget/installer.yaml.template" "$work_dir/$package_id.installer.yaml" \
  __PACKAGE_ID__ "$package_id" __VERSION__ "$version" \
...
release_render_template "$release_repository_root/packaging/winget/locale.en-US.yaml.template" "$work_dir/$package_id.locale.en-US.yaml" \
  __PACKAGE_ID__ "$package_id" __VERSION__ "$version" __PUBLISHER__ "$publisher_yaml" \
