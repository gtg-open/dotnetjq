...
# NuGet uses SOURCE_DATE_EPOCH to populate the DOS timestamps in both ZIP
# headers of every nupkg entry. 1980-01-01 is the earliest timestamp that ZIP
# can represent, and overriding an inherited value keeps every release tool on
# the same byte-reproducible package representation.
...
release_require_native_dotnet_sdk() {
  local expected_rid=$1
  local sdk_info
  local sdk_rid
  sdk_info=$(DOTNET_CLI_UI_LANGUAGE=en-US dotnet --info) ||
    release_die 'could not inspect the dotnet SDK used for local tool installation'
  sdk_rid=$(printf '%s\n' "$sdk_info" | awk '$1 == "RID:" { sub(/\r$/, "", $2); print $2 }')
  [ "$sdk_rid" = "$expected_rid" ] ||
    release_die "local tool installation requires a native $expected_rid SDK; dotnet reports ${sdk_rid:-<missing RID>}"
}

release_pointer_any_payload() {
  # Field comparisons avoid the slash-in-character-class regexp rejected by
  # macOS awk. Include every file below tools/<tfm>/any, not just settings XML.
  awk -F / 'substr($0, length($0), 1) != "/" && $1 == "tools" && $2 != "" && $3 == "any" && NF >= 4 { print }'
}
