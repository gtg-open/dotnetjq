...
release_require_native_dotnet_sdk "$rid"
...
# Use the native SDK checked above. In SDK 10.0.400, an explicit --arch
# unconditionally downloads microsoft.netcore.app.host.<rid>, even when the
# architecture already matches. Omitting it uses the SDK's bundled template
# and preserves the strict local-only package feed. See the SDK source link
# in README.md; exact selected-package and executable checks remain below.
# Native packages advertise tools/any, whose framework version is zero. Make
# net10.0 explicit so the same SDK method does not mistake it for a pre-5/6
# managed tool and download an x64 compatibility apphost on ARM64 Windows/macOS.
...
case $rid in
  win-*)
    ...
    executable="$tool_path/dotnetjq.cmd"
    installed_native_executable=$(python3 -B "$release_script_dir/verify-windows-tool-shim.py" \
      --tool-directory "$tool_path" --rid "$rid" \
      --package "$packages/$package_id.$rid.$version.nupkg")
    if command -v cygpath >/dev/null 2>&1; then
      installed_native_executable=$(cygpath -au -- "$installed_native_executable")
    fi
    release_verify_binary_format "$installed_native_executable" "$rid"
