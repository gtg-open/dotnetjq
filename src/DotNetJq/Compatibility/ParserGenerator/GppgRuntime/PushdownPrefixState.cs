#nullable disable
// Source-integrated from springcomp/gppg 1.2.5 commit f4634057620757a38789b4d53df73817667d1842 (PushdownPrefixState.cs).
// Modifications: the top-level runtime type is internal to DotNetJq.dll;
// the pinned source's nullable/analyzer context is retained explicitly.
...
namespace StarodubOleg.GPPG.Runtime
{
	...
	{
		// jq's pinned Bison parser uses YYMAXDEPTH=10000. Keeping the same
		// ceiling prevents an unbounded managed allocation. The scanner adapter
		// separately accounts for GPPG's one-slot source-nesting difference.
		internal const int MaximumDepth = 10_000;
	...
	internal sealed class ParserStackExhaustedException : System.Exception
	{
	}
}
