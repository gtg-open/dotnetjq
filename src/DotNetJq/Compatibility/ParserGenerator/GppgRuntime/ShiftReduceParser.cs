#nullable disable
// Source-integrated from springcomp/gppg 1.2.5 commit f4634057620757a38789b4d53df73817667d1842 (ShiftReduceParser.cs).
...
// diagnostic-only reduction projection, Bison's four-token expected-list
// limit, and symbol ordering support jq-compatible diagnostics without
// changing parser acceptance or recovery.
#pragma warning disable CA1305, CA1854
...
namespace StarodubOleg.GPPG.Runtime
{
	...
	internal abstract class ShiftReduceParser<TValue, TSpan> where TSpan : IMerge<TSpan>, new()
	{
		...
		private void ReportError()
		{
			...
			State diagnosticState = ProjectDiagnosticState();
			List<int> expected = new List<int>();
			foreach (int terminal in diagnosticState.ParserTable.Keys)
			{
				if (terminal != errorToken)
					expected.Add(terminal);
			}
			expected.Sort(CompareTerminalSymbols);

			// Bison's YYARGS_MAX is five: one unexpected token plus at most
			// four expected alternatives.
			if (expected.Count <= 4)
			{
				bool first = true;
				foreach (int terminal in expected)
				{
					if (first)
		...
		private State ProjectDiagnosticState()
		{
			List<State> projected = new List<State>(StateStack.Depth);
			for (int index = 0; index < StateStack.Depth; index++)
				projected.Add(StateStack[index]);

			bool changed = false;
			int remainingSteps = PushdownPrefixState<State>.MaximumDepth;
			while (projected.Count != 0)
			{
				if (remainingSteps-- == 0)
					return FsaState;

				State state = projected[projected.Count - 1];
				if (state.ParserTable != null &&
					state.ParserTable.TryGetValue(errorToken, out int recoveryAction) &&
					recoveryAction > 0)
					return changed ? state : FsaState;

				int action = state.defaultAction;
				if (state.ParserTable != null &&
					state.ParserTable.TryGetValue(NextToken, out int explicitAction))
					action = explicitAction;

				// If the unexpected token becomes shiftable or acceptable, the
				// projection would alter language semantics. Discard it entirely.
				if (action > 0 || action == -1)
					return FsaState;

				if (action == 0)
				{
					action = UniqueReduction(state);
					if (action == 0)
						return changed ? state : FsaState;
				}

				Rule rule = rules[-action];
				int remaining = projected.Count - rule.RightHandSide.Length;
				if (remaining <= 0)
					return FsaState;

				State predecessor = projected[remaining - 1];
				if (predecessor.Goto == null ||
					!predecessor.Goto.TryGetValue(rule.LeftHandSide, out int nextState))
					return FsaState;

				projected.RemoveRange(remaining, projected.Count - remaining);
				if (projected.Count >= PushdownPrefixState<State>.MaximumDepth)
					return FsaState;
				projected.Add(states[nextState]);
				changed = true;
			}
		...
		private static int UniqueReduction(State state)
		{
			int reduction = 0;
			if (state.ParserTable == null)
				return reduction;

			foreach (int action in state.ParserTable.Values)
			{
				if (action >= -1)
					continue;
				if (reduction == 0)
					reduction = action;
				else if (reduction != action)
					return 0;
			}
			return reduction;
		}

		private static int CompareTerminalSymbols(int left, int right)
		{
			return TerminalSymbolOrder(left).CompareTo(TerminalSymbolOrder(right));
		}

		private static int TerminalSymbolOrder(int terminal)
		{
			// GPPG assigns named terminals from 127 upward in declaration order.
			if (terminal >= 127)
				return terminal - 127;

			// Bison places literal terminals after the named declarations in
			// first-appearance order for this grammar.
			int literalOrder;
			switch (terminal)
			{
				case '|': literalOrder = 0; break;
				case ',': literalOrder = 1; break;
				case '=': literalOrder = 2; break;
				case '<': literalOrder = 3; break;
				case '>': literalOrder = 4; break;
				case '+': literalOrder = 5; break;
				case '-': literalOrder = 6; break;
				case '*': literalOrder = 7; break;
				case '/': literalOrder = 8; break;
				case '%': literalOrder = 9; break;
				case '?': literalOrder = 10; break;
				case '.': literalOrder = 11; break;
				case '[': literalOrder = 12; break;
				case ';': literalOrder = 13; break;
				case ':': literalOrder = 14; break;
				case '(': literalOrder = 15; break;
				case ')': literalOrder = 16; break;
				case ']': literalOrder = 17; break;
				case '{': literalOrder = 18; break;
				case '}': literalOrder = 19; break;
				case '$': literalOrder = 20; break;
				default: literalOrder = 1000 + terminal; break;
			}
			return 1000 + literalOrder;
		}
