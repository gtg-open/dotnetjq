...
try {
    ...
    & $CliPath -e -n false 1>$null 2>$null
    Assert-Equal 1 $LASTEXITCODE 'false exit status'
    & $CliPath -e -n empty 1>$null 2>$null
    Assert-Equal 4 $LASTEXITCODE 'no-output exit status'
    & $CliPath -n 'this is not valid jq syntax!' 1>$null 2>$null
    Assert-Equal 3 $LASTEXITCODE 'compile-error exit status'
    & $CliPath -n 'error("powershell-test")' 1>$null 2>$null
    Assert-Equal 5 $LASTEXITCODE 'runtime-error exit status'
}
finally {
