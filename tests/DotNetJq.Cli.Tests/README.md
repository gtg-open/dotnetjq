...
`artifacts/test-assets/jq-1.8.2/oracle/jq` when present, or the executable named by
`DOTNETJQ_JQ182`. The harness verifies `jq --version` before using an oracle.
...
`DOTNETJQ_UPSTREAM`. Outside strict mode it may fall back to the sibling `jq`
