...
| `HOMEBREW_APP_PRIVATE_KEY` | optional environment secret | GitHub App private key; when absent, the verified formula remains in the GitHub release and tap submission is deferred |
| `WINGET_CREATE_GITHUB_TOKEN` | optional environment secret | dedicated classic token with the `public_repo` scope accepted by `wingetcreate`; when absent, the verified manifests remain in the GitHub release and submission is deferred |
...
WinGet when their optional publisher credentials are configured. Missing
Homebrew or WinGet credentials never weaken or block GitHub and NuGet
publication: the exact formula and manifests stay in the immutable GitHub
release for a later, separately authorized submission. A configured credential
is still validated before core publication so an invalid credential cannot
create a partially distributed stable release. The Homebrew formula becomes
