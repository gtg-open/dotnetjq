...
executable under test. Before accessing `Process.StandardInput.BaseStream`, the
PowerShell byte probes select a BOM-less `UTF8Encoding` through
`ProcessStartInfo.StandardInputEncoding` when available. Windows PowerShell
5.1's .NET Framework does not expose that property, so the harness temporarily
sets `[Console]::InputEncoding`; .NET Framework uses it when constructing the
redirected `StandardInput` writer. This prevents that writer from injecting its
own UTF-8 preamble into otherwise raw test input. The original console encoding
is restored in `finally`. The probes drain stdout and stderr concurrently and
use a bounded process wait so a failed implementation cannot deadlock the
Windows job.
